...
def test_load_default_config():
    ...
    assert config.game.name == 'test-season'
    assert config.game.rounds == 10
    assert config.game.turn_limit == 6
    assert config.game.score_threshold == 8
    assert config.draft.picks_per_team == 3
    assert config.draft.style == 'snake'
    assert config.teams.red.name == 'Red Team'
    assert config.teams.blue.name == 'Blue Team'
    assert config.teams.judge.model_name == 'mistral-nemo:latest'
