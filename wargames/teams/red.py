...
from wargames.models import BugReport, Domain, Severity
...
class RedTeamAgent:
    ...
    async def attack(
        self, target: str, tools: list[str], past_lessons: list[str], strategies: list[str] = None
    ) -> str:
        """Generate an attack description against the target."""
        lessons_text = (
            '\n'.join(f'- {lesson}' for lesson in past_lessons) if past_lessons else 'None yet.'
        )
        ...
        return await self.llm.chat(
            [
                {
                    'role': 'user',
                    'content': f'Target system: {target}\n\nDescribe your attack in detail.',
                }
            ],
            system=system,
        )
    ...
    async def generate_bug_report(
        self, attack_desc: str, target: str, tools: list[str]
    ) -> BugReport:
        ...
        try:
            ...
            return BugReport(
                round_number=0,
                title='Unstructured Attack',
                severity=Severity.LOW,
                domain=Domain.MIXED,
                target=target,
                steps_to_reproduce=attack_desc,
                proof_of_concept='',
                impact='See attack description',
            )

    async def write_debrief(
        self, attacks_summary: str, draft_picks: list[str], outcome: str
    ) -> str:
